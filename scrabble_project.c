...
void game1_function(void);
void game2_function(void);
int wordsFound=0;
int main()
{
  while(1)
  {
  int game_choice, repeat;
  printf("Enter 1 for scrabble game and 2 for jumble words game, and other to exit(DO NOT ENTER ANY CHARACTER TO EXIT!)\n");
  ...
  switch(game_choice)
{
  case 1:
    do{
    game1_function();
  printf("\nEnter 1 to continue playing this and 0 to exit: ");
  scanf("%d",&repeat);
  system("cls");
  }while(repeat);
break;
case 2:
  do{
    game2_function();
  printf("Enter 1 to continue playing this and 0 to exit: ");
  scanf("%d",&repeat);
  system("cls");
  }while(repeat);
break;
default:
    printf("ALL THE BEST HOPE WE WILL MEET AGAIN");
    return 0;
}
}
}
...
void game1_function()
{
  ...
  printf("Six correct words need to be formed...\n");
  printf("Both players together need to form six words, after that game will display winner...\n");
 ...
 void game2_function()
  ...
  printf("You can start with easy words, and if you do well, you can progress to medium and then hard levels or directly go to the level of your choice\n");
  ...
  char easyWords[WORD_COUNT_EASY][WORD_LENGTH] = {
    "apple", "banana", "cherry", "kick", "elephant",
    "fish", "grape", "house", "igloo", "jazz",
    "kite", "quiz", "monkey", "nest", "orange",
  ...
  char hardWords[WORD_COUNT_HARD][WORD_LENGTH] = {
    ...
    "sequence", "innovation", "imagination", "enthusiasm", "dedication",
...
void game_rules(void){
  printf("There are alphabates given in upper case from A to Z(Both upper or lower case letter can be entered by players) :\n");
